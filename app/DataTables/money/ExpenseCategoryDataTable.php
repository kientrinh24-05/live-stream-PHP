...
use App\Http\Requests\Search\SExpenseCategory;
...
class ExpenseCategoryDataTable extends DataTable
{
    ...
    public function dataTable(SExpenseCategory $expenseCategory, $query)
    {
        $dem = $this->getCountTable('expense_categories', '', '', '', 'name', '');
        $count = $dem->count();
        ...
        return datatables()
            ...
            ->filter(function ($query) {
                ...
                if (request('start_date') != '' && request('end_date') != '' && request('end_date') >= request('start_date')) {
                    $query->whereBetween('expense_categories.created_at', [request('start_date') . " 00:00:00", request('end_date') . " 23:59:59"]);
                }
            })
            ...
            ->addColumn('action', function ($row) {
                return '<a href="javascript:void(0)" data-id="'.$row->id.'" class="btn btn-sm btn-white action_edit">
                        ...
                        data-url="transaction/expense/category/destroy/'.$row->id.'"><i class="tio-delete-outlined"></i> Delete</a>';
            })
    ...
    {
        return DB::table('expense_categories')->orderBy('id', 'desc');
    }
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::checkbox(),
            Column::make('name')->title('Tên chi phí')->orderable(false)->searchable(false),
