...
<div class="form-group required">
    ...
    <div id="image_show" class="text-center mt-lg-4 mt-sm-4">
        @if (Agent::isMobile())
            <img
                src="{!! ${$value}->{$name_img} != '' ? ${$value}->{$name_img} : asset('assets/svg/illustrations/browse.svg') !!} "
                id="{{$name_img}}" width="300px" height="300px" alt="{{$label}}">
        @else
            <img
                src="{!! ${$value}->{$name_img} != '' ? ${$value}->{$name_img} : asset('assets/svg/illustrations/browse.svg') !!} "
                id="{{$name_img}}" width="370px" height="370px" alt="{{$label}}">
        @endif
        <input type="hidden" name="{{$name_img}}" value="{{ ${$value}->{$name_img} }}">
    </div>
</div>
