...
abstract class DataTable implements DataTableButtons
{
    ...
    public function printPreview()
    {
        ...
        // Ghi nhật ký
        $this->logExport('download', 'print', $this->getFilename());
    ...
    public function excel()
    {
        ...
        // Ghi nhật ký
        $this->logExport('download', 'excel', $this->getFilename());
    ...
    public function csv()
    {
        ...
        // Ghi nhật ký
        $this->logExport('download', 'csv', $this->getFilename());
    ...
    public function pdf()
    {
        if ('snappy' == config('datatables-buttons.pdf_generator', 'snappy')) {
            // Ghi nhật ký
            $this->logExport('download', 'pdf', $this->getFilename());
    ...
    protected function logExport($name, $type, $data)
    {
        $info = [
            'data' => $this->getCauseType(),
            'file name' => $data,
            'export' => $type
        ];
        activity($name)
            ->withProperty($name, $info)
            ->tap(function (Activity $activity) {
                $activity->causer_type = $this->getCauseType();
    ...
    {
        $replace = Str::replace('DataTable', '', class_basename($this));
        $head = Str::headline($replace);
        $snake = Str::snake($head);
        return Str::title($snake);
    }
}
